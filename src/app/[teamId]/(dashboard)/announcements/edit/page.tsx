...
export default function EditAnnouncementPage() {
  ...
  useEffect(() => {
    ...
    if (!id) { router.replace(teamLink("/members")); return; }

    async function loadData() {
      setIsLoading(true);
      const ann = await fetchAnnouncementById(id!);
      if (!ann) { alert("お知らせが見つかりませんでした。"); router.replace(teamLink("/members")); return; }
      setTitle(ann.title);
      setBody(ann.body);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (updated) { router.push(teamLink("/members")); }
  ...
  return (
    <div className="px-4 py-4 space-y-4 pb-24">
      <Link href={teamLink("/members")} className="inline-flex items-center gap-1 text-[13px] text-muted active:text-primary transition-colors py-1 pr-2">
        <ArrowLeft className="w-4 h-4" />管理者メニューに戻る
      </Link>
      <h1 className="font-black text-lg flex items-center gap-1.5"><Megaphone className="w-5 h-5 text-primary" />お知らせを編集</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="pt-2">
          <button type="submit" disabled={isSaving || !title.trim() || !body.trim()} className={cn("w-full flex items-center justify-center gap-2 py-4 rounded-2xl font-black text-[15px] transition-all outline-none", isSaving || !title.trim() || !body.trim() ? "bg-primary/30 text-white cursor-not-allowed" : "bg-primary text-white active:bg-primary-light shadow-lg active:scale-[0.98] touch-active")}>
            <Save className="w-5 h-5" />{isSaving ? "更新中..." : "お知らせを更新する"}
          </button>
        </div>
      </form>
    </div>
