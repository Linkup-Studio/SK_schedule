...
export default function AdminMenuPage() {
  ...
  const handleDeleteAnnouncement = async (id: string) => {
    ...
    if (!success) {
      alert("削除に失敗しました。");
      loadAnnouncements(); // 失敗したら元に戻すために再読み込み
    }
  };
      ...
      <section className="space-y-3 pt-4 border-t border-border/50">
        ...
        <div className="space-y-2">
          {loadingAnnouncements ? (
            ...
            <div className="bg-surface rounded-2xl border border-border p-6 text-center shadow-sm">
              <p className="text-[12px] font-bold text-muted">お知らせはありません</p>
            </div>
          ) : (
