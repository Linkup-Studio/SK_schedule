          ...
          <div className="space-y-3">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-3 gap-2">
                {([{ status: "attend" as AttendanceStatusValue, icon: "○", label: "参加", activeClass: "bg-attend text-white shadow-attend/30 border-attend" }, { status: "absent" as AttendanceStatusValue, icon: "×", label: "欠席", activeClass: "bg-absent text-white shadow-absent/30 border-absent" }, { status: "undecided" as AttendanceStatusValue, icon: "△", label: "未定", activeClass: "bg-undecided text-white shadow-undecided/30 border-undecided" }]).map((config) => (<button key={config.status} type="button" onClick={() => setSelectedStatus(config.status)} className={cn("flex flex-col items-center justify-center py-4 px-2 rounded-xl border-2 font-bold transition-all shadow-sm active:scale-95 outline-none touch-active", selectedStatus === config.status ? cn(config.activeClass, "scale-[1.02] shadow-md") : "border-border bg-white text-muted hover:bg-surface-variant active:bg-border")}><span className="text-2xl font-black">{config.icon}</span><span className="text-[10px] mt-0.5">{config.label}</span></button>))}
              </div>
